...
window.addEventListener('load', () => {
  ...
  updatePhoto();
  // Auto play music when gallery opened
  fadeInMusic();
});
...
function fadeInMusic() {
  ...
  galleryMusic.volume = 0;
  galleryMusic.play().catch(() => {}); // aman dari autoplay error

  musicFadeInterval = setInterval(() => {
    if (galleryMusic.volume < MAX_VOLUME - 0.02) {
      galleryMusic.volume += 0.02;   // naik pelan
    } else {
      galleryMusic.volume = MAX_VOLUME; // 🎚️ stop di 0.3
      clearInterval(musicFadeInterval);
    }
  }, 100);
}
...
function fadeOutMusic(callback) {
  ...
  musicFadeInterval = setInterval(() => {
    if (galleryMusic.volume > 0.05) {
      galleryMusic.volume -= 0.05;
    } else {
      galleryMusic.volume = 0;
      galleryMusic.pause();
